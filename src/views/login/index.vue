  ...
  <AppPage :show-footer="true" bg-cover :style="{ backgroundImage: `url(${bgImg})` }">
    ...
    >
      ...
      <div w-320 flex-col px-20 py-35>
        ...
        <div mt-30>
          <n-input
            v-model:value="loginInfo.encryptedPwd"
            class="text-16 items-center h-50 pl-10"
            type="password"
...
function initLoginInfo() {
  ...
  if (localLoginInfo) {
    ...
    loginInfo.value.encryptedPwd = localLoginInfo.encryptedPwd || ''
  }
}
...
async function handleLogin() {
  const { username, encryptedPwd } = loginInfo.value
  ...
  try {
    ...
    const res = await api.login({ username, encryptedPwd: encryptedPwd.toString() })
    $message.success('登录成功')
    setToken(res.data.token)
    if (isRemember.value) {
      lStorage.set('loginInfo', { username, encryptedPwd })
    } else {
